...
try:
    ...
    logger.warning("VisualValidator not available: No module named 'visual_validator'")
                        ...
                        if max_abs > 0:
                            ...
                            try:
                                ...
                                if max_abs > 0 and normalized_energy > 0:
                                    ...
                                    log_energy = np.log10(normalized_energy) + 2 * np.log10(max_abs)
...
class WaveformAnalyzer(QObject):
    ...
    def __init__(self):
        ...
        if MADMOM_AVAILABLE:
            ...
            # Use a more optimized configuration for the OnsetPeakPickingProcessor
            # Lower threshold for better sensitivity, with pre_max and post_max for precise timing
            self.onset_peak_picker = OnsetPeakPickingProcessor(
                threshold=0.28,  # Slightly lower threshold for better sensitivity
                pre_max=0.03,  # Look 30ms before for rising edge detection
                post_max=0.03,  # Look 30ms after to ensure we catch the true peak
                pre_avg=0.1,  # Use 100ms for pre-peak average calculation
                post_avg=0.1,  # Use 100ms for post-peak average calculation
                combine=0.05,  # Combine onsets within 50ms
        ...
        self.config = {
            ...
            'min_peak_distance': 0.03,  # 30ms to avoid detecting multiple peaks for the same drum hit
            'cluster_refinement_distance': 0.04,  # 40ms for better separation of drum hits
            ...
            'peaks_per_minute': 120,  # Expected maximum number of peaks per minute of audio

            # Adaptive Processing Settings
            'enable_adaptive_processing': True,  # Enable adaptive processing for large files
            'chunk_size_seconds': 60,  # Process long files in 60-second chunks
            'overlap_seconds': 5,  # Overlap between chunks to ensure continuity
            ...
            'percussive_separation': True,  # Critical for drum detection
            ...
            'madmom_onset_threshold': 0.4,  # Selective threshold for quality detections
            'madmom_fps': 100,
            'madmom_beat_tracking_method': 'default',  # 'default', 'comb', 'acf'
            'madmom_tempo_range': (60, 200),  # Typical drum pattern BPM range,

            # Visual Validation Settings
            'use_visual_validation': False  # Disabled by default
        }
    ...
    def validate_analysis_results(self) -> Dict[str, Any]:
        ...
        validation_report = self.visual_validator.validate_peaks(peak_dicts, self.waveform_data, self.sample_rate)
