...
# Import madmom for enhanced audio analysis
try:
    import madmom
    from madmom.audio import Signal as MadmomSignal, FramedSignal
    ...
    print("✅ madmom successfully imported - enhanced audio analysis enabled")
except ImportError as e:
    MADMOM_AVAILABLE = False
    print(f"⚠️ madmom not available - some advanced features disabled: {str(e)}")
...
class AdvancedSignalProcessor:
    ...
    def enhanced_onset_strength(self, signal: np.ndarray, sr: int) -> np.ndarray:
        """
        Compute enhanced onset strength using multiple methods including madmom
        """
        try:
            # Compute STFT
            ...
            # Method 5: Madmom onset detection (if available)
            madmom_onset = np.zeros_like(onset_1)  # Default empty array
            if MADMOM_AVAILABLE:
                try:
                    # Create a temporary MadmomIntegration instance
                    madmom_processor = MadmomIntegration(sample_rate=sr)

                    # Get madmom signal
                    mm_signal = madmom_processor.process_signal(signal)

                    # Use spectral onset processor for compatibility with our array sizes
                    spectral_processor = SpectralOnsetProcessor()
                    madmom_activations = spectral_processor(mm_signal)

                    # Resample to match our array size if needed
                    if len(madmom_activations) > 0:
                        from scipy.interpolate import interp1d
                        x_orig = np.linspace(0, 1, len(madmom_activations))
                        x_new = np.linspace(0, 1, len(onset_1))
                        f = interp1d(x_orig, madmom_activations, bounds_error=False, fill_value='extrapolate')
                        madmom_onset = f(x_new)

                        # Normalize
                        if np.max(madmom_onset) > 0:
                            madmom_onset = madmom_onset / np.max(madmom_onset)
                except Exception as e:
                    logger.warning(f"Error in madmom onset detection: {e}")
            ...
            # Combine methods with weights (including madmom if available)
            if MADMOM_AVAILABLE and np.any(madmom_onset):
                # Give more weight to madmom if available
                combined_onset = (0.2 * onset_1 + 0.2 * flux + 0.2 * hfc + 0.15 * complex_onset + 0.25 * madmom_onset)
            else:
                # Original weights if madmom not available
...
class NoiseFilter:
    ...
    def multi_stage_denoising(self, signal: np.ndarray) -> np.ndarray:
        """
        Apply multi-stage denoising for optimal results with madmom integration
        """
        try:
            ...
            from scipy.ndimage import gaussian_filter1d
            stage4 = gaussian_filter1d(stage3, sigma=0.5)

            # Stage 5: Madmom-based enhancement (if available)
            if MADMOM_AVAILABLE:
                try:
                    # Create a temporary MadmomIntegration instance
                    madmom_processor = MadmomIntegration(sample_rate=self.sr)

                    # Use madmom's filtered spectrogram for noise reduction
                    mm_signal = madmom_processor.process_signal(stage4)

                    # Get filtered spectrogram
                    filtered_spec = madmom_processor.get_filtered_spectrogram(stage4)

                    if len(filtered_spec) > 0:
                        # Use the filtered spectrogram to create a mask for the original signal
                        # This is a simplified approach - in a real implementation, we would
                        # do more sophisticated processing with the spectrogram

                        # Convert filtered spectrogram to time domain
                        from madmom.audio.signal import Signal as MadmomSignal
                        from madmom.audio.stft import ShortTimeFourierTransform

                        # Create framed signal
                        frames = madmom.audio.FramedSignal(mm_signal, frame_size=2048, hop_size=441)

                        # Compute STFT
                        stft = ShortTimeFourierTransform(frames)

                        # Apply gentle noise reduction in frequency domain
                        # (simplified approach - just for demonstration)
                        stft_modified = stft.copy()

                        # Apply the inverse STFT to get back to time domain
                        # Note: This is a simplified approach and may not work perfectly
                        # In a real implementation, we would use proper ISTFT
                        enhanced_signal = stage4

                        # Blend with original signal
                        final_signal = 0.7 * enhanced_signal + 0.3 * stage4
                        return final_signal
                except Exception as e:
                    logger.warning(f"Error in madmom-based denoising: {e}")
                    return stage4
...
class MadmomIntegration:
    """
    Integrates madmom library functionality for enhanced audio analysis

    This class provides methods that leverage madmom's advanced audio processing
    capabilities, including neural network-based onset detection, beat tracking,
    and spectral analysis.
    """

    def __init__(self, sample_rate: int = 44100):
        """
        Initialize the madmom integration

        Args:
            sample_rate: Audio sample rate in Hz
        """
        self.sample_rate = sample_rate
        self.fps = 100  # Frames per second for madmom processing
        self._onset_processors = {}
        self._beat_processors = {}

    def process_signal(self, signal: np.ndarray) -> madmom.audio.Signal:
        """
        Convert numpy array to madmom Signal object

        Args:
            signal: Audio signal as numpy array

        Returns:
            madmom.audio.Signal object
        """
        return MadmomSignal(signal, sample_rate=self.sample_rate)

    def detect_onsets_cnn(self, signal: np.ndarray) -> np.ndarray:
        """
        Detect onsets using madmom's CNN onset processor

        Args:
            signal: Audio signal as numpy array

        Returns:
            Array of onset detection function values
        """
        ...
        try:
            ...
            mm_signal = self.process_signal(signal)

            # Use CNN onset processor
            if 'cnn' not in self._onset_processors:
                self._onset_processors['cnn'] = CNNOnsetProcessor()

            # Get onset activation function
            activations = self._onset_processors['cnn'](mm_signal)

            # Peak picking
            onset_processor = OnsetPeakPickingProcessor(threshold=0.3, fps=self.fps)
            onsets = onset_processor(activations)

            return onsets

        except Exception as e:
            logger.warning(f"Error in CNN onset detection: {e}")
            return np.array([])

    def detect_onsets_rnn(self, signal: np.ndarray) -> np.ndarray:
        """
        Detect onsets using madmom's RNN onset processor
        ...
        try:
            ...
            # Use RNN onset processor
            if 'rnn' not in self._onset_processors:
                self._onset_processors['rnn'] = RNNOnsetProcessor()

            # Get onset activation function
            activations = self._onset_processors['rnn'](mm_signal)

            # Peak picking
            onset_processor = OnsetPeakPickingProcessor(threshold=0.25, fps=self.fps)
            onsets = onset_processor(activations)

            return onsets

        except Exception as e:
            logger.warning(f"Error in RNN onset detection: {e}")
            return np.array([])

    def detect_onsets_spectral(self, signal: np.ndarray) -> np.ndarray:
        """
        Detect onsets using madmom's spectral onset processor
        ...
        try:
            ...
            # Use spectral onset processor
            if 'spectral' not in self._onset_processors:
                self._onset_processors['spectral'] = SpectralOnsetProcessor()

            # Get onset activation function
            activations = self._onset_processors['spectral'](mm_signal)

            # Peak picking
            onset_processor = OnsetPeakPickingProcessor(threshold=0.2, fps=self.fps)
            onsets = onset_processor(activations)

            return onsets

        except Exception as e:
            logger.warning(f"Error in spectral onset detection: {e}")
            return np.array([])

    def detect_beats(self, signal: np.ndarray) -> np.ndarray:
        """
        Detect beats using madmom's RNN beat processor
        ...
        try:
            ...
            if 'rnn_beat' not in self._beat_processors:
                self._beat_processors['rnn_beat'] = RNNBeatProcessor()

            # Get beat activation function
            activations = self._beat_processors['rnn_beat'](mm_signal)

            # Beat tracking
            beats_processor = BeatTrackingProcessor(fps=self.fps)
            beats = beats_processor(activations)

            return beats

        except Exception as e:
            logger.warning(f"Error in beat detection: {e}")
            return np.array([])

    def get_filtered_spectrogram(self, signal: np.ndarray) -> np.ndarray:
        """
        Compute filtered spectrogram using madmom

        Args:
            signal: Audio signal as numpy array

        Returns:
            Filtered spectrogram as numpy array
        """
        if not MADMOM_AVAILABLE:
            return np.array([])
        ...
        try:
            ...
            # Create framed signal
            frames = FramedSignal(mm_signal, frame_size=2048, hop_size=441)
            ...
            filt_spec = FilteredSpectrogram(spec)

            return filt_spec

        except Exception as e:
            logger.warning(f"Error computing filtered spectrogram: {e}")
            return np.array([])

    def get_onset_features(self, signal: np.ndarray, onset_times: np.ndarray) -> List[Dict[str, float]]:
        """
        Extract features at onset positions using madmom
        ...
        """
        if not MADMOM_AVAILABLE or len(onset_times) == 0:
            return []
        ...
        try:
            ...
            # Compute filtered spectrogram
            spec = self.get_filtered_spectrogram(signal)
            if len(spec) == 0:
                return []

            # Extract features at onset positions
            features = []
            for onset_time in onset_times:
                # Convert time to frame index
                frame_idx = int(onset_time * self.fps)
                if 0 <= frame_idx < len(spec):
                    # Extract spectral features at onset
                    onset_features = {
                        'spectral_centroid': np.mean(spec[frame_idx]),
                        'spectral_flux': np.sum(np.diff(np.vstack((spec[max(0, frame_idx-1)], spec[frame_idx])), axis=0)),
                        'spectral_flatness': np.exp(np.mean(np.log(spec[frame_idx] + 1e-10))) / (np.mean(spec[frame_idx]) + 1e-10)
                    }
                    features.append(onset_features)

            return features

        except Exception as e:
            logger.warning(f"Error extracting onset features: {e}")
            return []
...
class WaveformAnalyzer(QObject):
    ...
    def __init__(self):
        ...
        # Initialize madmom integration if available
        self.madmom_integration = MadmomIntegration() if MADMOM_AVAILABLE else None
    ...
    def _preprocess_signal(self) -> np.ndarray:
        ...
        # Apply madmom processing if available
        if MADMOM_AVAILABLE and self.madmom_integration is not None:
            try:
                print(f"   🔄 Applying madmom signal processing...")

                # Get filtered spectrogram using madmom
                filtered_spec = self.madmom_integration.get_filtered_spectrogram(signal)

                if len(filtered_spec) > 0:
                    print(f"   ✅ Madmom filtered spectrogram computed: shape={filtered_spec.shape}")

                    # Detect beats using madmom for rhythm-aware processing
                    beats = self.madmom_integration.detect_beats(signal)
                    if len(beats) > 0:
                        print(f"   ✅ Madmom beat detection found {len(beats)} beats")
                        # Store beats for later use in drum classification
                        if not hasattr(self, 'detected_beats'):
                            self.detected_beats = []
                        self.detected_beats = beats
                else:
                    print(f"   ⚠️ Madmom filtered spectrogram computation failed")
            except Exception as e:
                print(f"   ⚠️ Madmom signal processing failed: {e}")
                logger.warning(f"Madmom signal processing failed: {e}")
    ...
    def _detect_onsets_multi_method(self, signal: np.ndarray) -> List[Dict[str, Any]]:
        ...
        # Method 1: Madmom onset detection (if available)
        if MADMOM_AVAILABLE and self.madmom_integration is not None:
            print(f"   🔄 Starting Method 1: Madmom neural network-based onset detection...")
            try:
                # CNN-based onset detection
                print(f"      🧠 Using CNN onset detection...")
                cnn_onsets = self.madmom_integration.detect_onsets_cnn(working_signal)

                if len(cnn_onsets) > 0:
                    print(f"         ✅ Found {len(cnn_onsets)} CNN onset candidates")
                    # Convert to our candidate format
                    for onset_time in cnn_onsets:
                        # Scale time back to original if downsampled
                        if time_scale_factor > 1:
                            onset_time = onset_time * time_scale_factor

                        # Get amplitude at this time point
                        sample_idx = int(onset_time * self.sample_rate)
                        amplitude = abs(signal[sample_idx]) if 0 <= sample_idx < len(signal) else 0.0

                        onset_candidates.append({
                            'time': float(onset_time),
                            'strength': 0.8,  # High confidence for CNN detections
                            'amplitude': float(amplitude),
                            'confidence': 0.8,
                            'method': 'madmom_cnn',
                            'frequency_content': 0.0  # Will be filled in later
                        })
                else:
                    print(f"         ℹ️ No CNN onsets detected")

                # RNN-based onset detection
                print(f"      🧠 Using RNN onset detection...")
                rnn_onsets = self.madmom_integration.detect_onsets_rnn(working_signal)

                if len(rnn_onsets) > 0:
                    print(f"         ✅ Found {len(rnn_onsets)} RNN onset candidates")
                    # Convert to our candidate format
                    for onset_time in rnn_onsets:
                        # Scale time back to original if downsampled
                        if time_scale_factor > 1:
                            onset_time = onset_time * time_scale_factor
                        ...
                        onset_candidates.append({
                            'time': float(onset_time),
                            'strength': 0.75,  # High confidence for RNN detections
                            'amplitude': float(amplitude),
                            'confidence': 0.75,
                            'method': 'madmom_rnn',
                            'frequency_content': 0.0  # Will be filled in later
                    ...
                    print(f"         ℹ️ No RNN onsets detected")

                # Spectral-based onset detection
                print(f"      🎵 Using spectral onset detection...")
                spectral_onsets = self.madmom_integration.detect_onsets_spectral(working_signal)

                if len(spectral_onsets) > 0:
                    print(f"         ✅ Found {len(spectral_onsets)} spectral onset candidates")
                    # Convert to our candidate format
                    for onset_time in spectral_onsets:
                        # Scale time back to original if downsampled
                        if time_scale_factor > 1:
                            onset_time = onset_time * time_scale_factor
                        ...
                        onset_candidates.append({
                            'time': float(onset_time),
                            'strength': 0.7,  # Good confidence for spectral detections
                            'amplitude': float(amplitude),
                            'confidence': 0.7,
                            'method': 'madmom_spectral',
                            'frequency_content': 0.0  # Will be filled in later
                        })
                else:
                    print(f"         ℹ️ No spectral onsets detected")

            except Exception as e:
                print(f"      ⚠️ Madmom onset detection failed: {e}")
                logger.warning(f"Madmom onset detection failed: {e}")

        # Method 2: Librosa onset detection with multiple algorithms
        print(f"   🔄 Starting Method 2: Librosa onset detection...")
    ...
    def _classify_drum_hits(self, peaks: List[Dict[str, Any]], signal: np.ndarray) -> List[Dict[str, Any]]:
        """
        Enhanced drum classification with temporal consistency, advanced features, and madmom integration
        """
        logger.info("Enhanced drum hit classification with madmom integration...")
        start_time = time.time()

        classified_peaks = []

        # Get beat information if available (for rhythm-aware classification)
        beats = getattr(self, 'detected_beats', []) if MADMOM_AVAILABLE else []
        has_beat_info = len(beats) > 0
        if has_beat_info:
            logger.info(f"Using {len(beats)} detected beats for rhythm-aware classification")

        # Extract madmom features for all peaks if available
        madmom_features = []
        if MADMOM_AVAILABLE and self.madmom_integration is not None:
            try:
                # Extract onset times
                onset_times = np.array([peak['time'] for peak in peaks])

                # Get madmom features for these onsets
                madmom_features = self.madmom_integration.get_onset_features(signal, onset_times)

                if len(madmom_features) > 0:
                    logger.info(f"Extracted madmom features for {len(madmom_features)} peaks")
            except Exception as e:
                logger.warning(f"Error extracting madmom features: {e}")
                madmom_features = []
        ...
        for i, peak in enumerate(peaks):
            try:
                ...
                # Add madmom features if available
                if MADMOM_AVAILABLE and i < len(madmom_features):
                    for key, value in madmom_features[i].items():
                        features[f'madmom_{key}'] = value

                # Add beat-relative features if beat information is available
                if has_beat_info:
                    # Find closest beat
                    closest_beat_idx = np.argmin(np.abs(beats - peak['time']))
                    closest_beat_time = beats[closest_beat_idx]

                    # Calculate distance to closest beat (normalized by average beat interval)
                    if len(beats) > 1:
                        avg_beat_interval = np.mean(np.diff(beats))
                        beat_distance = abs(peak['time'] - closest_beat_time) / avg_beat_interval
                        features['beat_distance'] = beat_distance

                        # Is this likely a downbeat? (first beat in measure)
                        if closest_beat_idx % 4 == 0:  # Assuming 4/4 time signature
                            features['likely_downbeat'] = 1.0
                        else:
                            features['likely_downbeat'] = 0.0

                        # Position in beat (0.0 = on beat, 0.5 = halfway between beats)
                        features['beat_position'] = beat_distance if beat_distance <= 0.5 else 1.0 - beat_distance
                ...
                # Enhance confidence based on madmom detection method
                if 'method' in peak and peak['method'].startswith('madmom_'):
                    # Boost confidence for madmom-detected onsets
    ...
    def detect_beats_with_madmom(self, signal: np.ndarray = None) -> List[float]:
        """
        Detect beats in the audio signal using madmom

        Args:
            signal: Optional audio signal (uses stored waveform data if None)

        Returns:
            List of beat times in seconds
        """
        if not MADMOM_AVAILABLE or self.madmom_integration is None:
            logger.warning("Madmom not available for beat detection")
            return []

        try:
            # Use provided signal or get from waveform data
            if signal is None:
                if self.waveform_data is None or len(self.waveform_data) == 0:
                    logger.warning("No waveform data available for beat detection")
                    return []
                signal = self.waveform_data[0]

            # Detect beats using madmom
            beats = self.madmom_integration.detect_beats(signal)

            # Store beats for later use
            self.detected_beats = beats

            logger.info(f"Detected {len(beats)} beats using madmom")
            return beats.tolist()

        except Exception as e:
            logger.error(f"Error in madmom beat detection: {e}")
            return []
    ...
    def get_comprehensive_analysis_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "madmom_analysis": {
                ...
                "beat_detection": hasattr(self, 'detected_beats') and len(getattr(self, 'detected_beats', [])) > 0,
                "beat_count": len(getattr(self, 'detected_beats', [])) if hasattr(self, 'detected_beats') else 0,
                "beat_times": getattr(self, 'detected_beats', []).tolist() if hasattr(self, 'detected_beats') else []
            },
